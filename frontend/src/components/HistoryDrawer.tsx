            ...
            <div className="space-y-3">
              {history.map((item) => (
                <div
                  ...
                  className="p-4 rounded-2xl bg-goa-forest border border-goa-line/30 hover:border-goa-yellow/50 cursor-pointer transition-all hover:scale-[1.02] group"
                >
                  <div className="flex items-center justify-between text-[10px] font-mono text-goa-cream/50 mb-1">
                    <span>{item.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    <span className={item.data.refused ? 'text-goa-pink' : 'text-emerald-400 font-semibold'}>
                      {item.data.refused ? 'REFUSED' : 'GROUNDED'}
                    </span>
                  </div>
                  <h4 className="text-sm font-semibold text-goa-cream group-hover:text-goa-yellow transition-colors line-clamp-2">
                    "{item.query}"
                  </h4>
